...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    // Full disclosure: I wanted to add in a piece of code that would make it so that after you collected all of the coins,
    // you would have to get on a warp pad to take you to the next level. I wrote code to make it into an object as well (essentially duplicating
    // the coin code). Note that I also used:
    //
    //    Warp.prototype.type = "warp";
    //
    // I kept getting an error that said that the rungame function stopped getting input, so I took the loss on this one.
    //here's what I tried to do. Any help would be appriciated.
    /*
    else if (type == "warp") {
      if (!this.actors.some(function(actor) {
           return actor.type == "coin";
